...
pattern = re.compile(r'@app\.route\(\'/api/auto-company-import\', methods=\[\'POST\'\]\)\ndef auto_company_import\(\):(.*?)(?=\ndef |\n@app.route|\Z)', re.DOTALL)
...
if pattern.search(content):
    content = pattern.sub(lambda m: new_route, content)
    print("Replaced existing auto_company_import route safely.")
else:
    if "if __name__ == '__main__':" in content:
        content = content.replace("if __name__ == '__main__':", new_route + "\n\nif __name__ == '__main__':")
    else:
        content += "\n" + new_route
    print("Appended new auto_company_import route.")
