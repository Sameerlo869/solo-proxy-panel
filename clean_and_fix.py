...
# 1. Remove all duplicate or old auto_ingest_request definitions
content = re.sub(r"@app\.route\('/admin/auto-ingest', methods=\['POST'\]\).*?return jsonify.*?500\n\s*", "", content, flags=re.DOTALL)
content = re.sub(r"def auto_ingest_request\(.*?\n\s*return.*?\n", "", content, flags=re.DOTALL)

# 2. Remove old broken widgets
content = re.sub(r"\s*<!-- Smart Quick Auto-Ingest Widget -->.*?(?=</body>)", "", content, flags=re.DOTALL)

# 3. Append a single, clean auto_ingest_request endpoint at the bottom
clean_route = """
...
# 4. Clean UI Widget to inject right inside <div class="main">
proper_widget = """
...
print("App cleaned and fully fixed!")
