...
# [Auto-Cleaned CSS] GIST_URL = f"https://api.github.com/gists/{GIST_ID}"
BACKUP_FILE = "backup.json"

DEFAULT_DB = {
    "admin_u": "admin", "admin_p": generate_password_hash("admin"), 
# [Auto-Cleaned CSS]     "companies": {"ramfin": {"name": "RamFincorp", "tenant": "MMMWO", "broker": "ramfin", "active": True}},
    "services": {
        "pan_ramfin": {
...
# [Auto-Cleaned CSS]             "method": "POST", "headers": {"authorization": "Bearer {{token}}", "x-tenant": "{{tenant}}"},
# [Auto-Cleaned CSS]             "body_template": {"panNumber": "{{pan}}"}, "query_params": {},
            "auth_token": "", "timeout": 10, "active": True, "health_status": True, "health_last_check": 0
        }
    },
# [Auto-Cleaned CSS]     "keys": {}, "logs": [], "audit_logs": []
}
...
def add_company(code, name, tenant, broker, active=True):
    db = GistDB.load()
# [Auto-Cleaned CSS]     db.setdefault('companies', {})
    if code in db['companies']: return False
# [Auto-Cleaned CSS]     db['companies'][code] = {"name": name, "tenant": tenant, "broker": broker, "active": bool(active)}
    GistDB.save(db)
    return True
...
# [Auto-Cleaned CSS]     res = {"method": "GET", "base_url": "", "endpoint": "", "headers": {}, "body_template": {}, "query_params": {}, "auth_token": ""}
    url_m = re.search(r"(https?://[^\s'\"\\\\]+)", raw_text)
    if url_m:
        parsed_u = urllib.parse.urlparse(url_m.group(1))
# [Auto-Cleaned CSS]         res["base_url"] = f"{parsed_u.scheme}://{parsed_u.netloc}"
...
# [Auto-Cleaned CSS]                 res["headers"][k] = "Bearer {{token}}"
...
def add_service(code, service_data):
    db = GistDB.load()
# [Auto-Cleaned CSS]     db.setdefault('services', {})
...
def generate_key(owner, days, limit, assigned_services):
    db = GistDB.load()
# [Auto-Cleaned CSS]     db.setdefault('keys', {})
# [Auto-Cleaned CSS]     k = f"KEY_{secrets.token_hex(4).upper()}"
    db['keys'][k] = {
...
# [Auto-Cleaned CSS]     db.setdefault('logs', []).append({"ts": now_ts(), "key": key, "service": service, "ok": ok, "msg": msg})
    db['logs'] = db['logs'][-500:]
    GistDB.save(db)
...
# [Auto-Cleaned CSS]     key_obj = db.get('keys', {}).get(k)
...
# [Auto-Cleaned CSS]     srv_obj = db.get('services', {}).get(srv)
...
# [Auto-Cleaned CSS]     cmp_obj = db.get('companies', {}).get(srv_obj.get('company', ''))
...
<style>
# [Auto-Cleaned CSS] :root{--bg:#0b0c10;--glass:rgba(31,40,51,0.6);--border:rgba(69,162,158,0.3);--neon:#66fcf1;--neon-dim:#45a29e;}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] body{background:var(--bg);color:#c5c6c7;font-family:'Segoe UI',sans-serif;margin:0;display:flex;height:100vh;overflow:hidden;}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] ::-webkit-scrollbar{width:6px;} ::-webkit-scrollbar-thumb{background:var(--neon-dim);border-radius:3px;}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .glass{background:var(--glass);backdrop-filter:blur(12px);border:1px solid var(--border);border-radius:12px;box-shadow:0 4px 6px rgba(0,0,0,0.3);}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .sidebar{width:260px;padding:20px;border-right:1px solid var(--border);display:flex;flex-direction:column;gap:10px;}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .logo{font-size:24px;color:var(--neon);text-shadow:0 0 10px var(--neon);margin-bottom:20px;font-weight:bold;text-align:center;}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .nav-item{padding:12px 15px;cursor:pointer;border-radius:8px;transition:0.3s;display:flex;align-items:center;gap:12px;}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .nav-item:hover, .nav-item.active{background:rgba(102,252,241,0.1);color:var(--neon);box-shadow:inset 4px 0 0 var(--neon);}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .main{flex:1;padding:25px;overflow-y:auto;position:relative;}
# [Auto-Cleaned CSS] # [Cleaned CSS] .topbar{display:flex;justify-content:space-between;align-items:center;margin-bottom:30px;padding-bottom:15px;border-bottom:1px solid var(--border);}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .clock{font-size:18px;color:var(--neon);text-shadow:0 0 5px var(--neon);letter-spacing:1px;font-family:monospace;}
# [Auto-Cleaned CSS] # [Cleaned CSS] .grid-4{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:20px;margin-bottom:20px;}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .card{padding:20px;transition:0.3s;} .card:hover{transform:translateY(-3px);box-shadow:0 0 15px rgba(102,252,241,0.2);border-color:var(--neon-dim);}
# [Auto-Cleaned CSS] # [Cleaned CSS] .tab{display:none;animation:fade 0.4s;} .tab.active{display:block;}
# [Auto-Cleaned CSS] @keyframes fade{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .health-dot{width:12px;height:12px;border-radius:50%;display:inline-block;animation:pulse 1.5s infinite;}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .green{background:#5cb85c;box-shadow:0 0 10px #5cb85c;} .red{background:#d9534f;box-shadow:0 0 10px #d9534f;}
# [Auto-Cleaned CSS] @keyframes pulse{0%{transform:scale(0.95);opacity:0.8}50%{transform:scale(1.1);opacity:1}100%{transform:scale(0.95);opacity:0.8}}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .table{width:100%;border-collapse:collapse;margin-top:10px;font-size:14px;} .table th,.table td{padding:12px;text-align:left;border-bottom:1px solid rgba(255,255,255,0.05);}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .btn{padding:8px 15px;border:none;border-radius:6px;cursor:pointer;color:#0b0c10;font-weight:bold;background:var(--neon);transition:0.3s;text-decoration:none;display:inline-block;}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .btn:hover{box-shadow:0 0 12px var(--neon);} .btn-danger{background:rgba(217,83,79,0.2);color:#d9534f;border:1px solid #d9534f;} .btn-danger:hover{box-shadow:0 0 12px #d9534f;background:#d9534f;color:#fff;}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] input, select, textarea{width:100%;padding:10px;margin:8px 0 15px;background:rgba(0,0,0,0.4);border:1px solid var(--border);color:#fff;border-radius:6px;outline:none;}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] input:focus, textarea:focus{border-color:var(--neon);box-shadow:0 0 8px rgba(102,252,241,0.4);}
# [Auto-Cleaned CSS] # [Naked CSS Nuked] .modal{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:99;justify-content:center;align-items:center;}
</style></head><body>
...
<div class="main">
    ...
    <div id="dash" class="tab active">
        <div class="grid-4">
# [Auto-Cleaned CSS] # [Naked CSS Nuked]             <div class="card glass"><h4><i class="fa-solid fa-building"></i> Partners</h4><h2 style="color:var(--neon)">{{ db.get('companies',{})|length }}</h2></div>
# [Auto-Cleaned CSS] # [Naked CSS Nuked]             <div class="card glass"><h4><i class="fa-solid fa-network-wired"></i> Gateways</h4><h2 style="color:var(--neon)">{{ db.get('services',{})|length }}</h2></div>
# [Auto-Cleaned CSS] # [Naked CSS Nuked]             <div class="card glass"><h4><i class="fa-solid fa-users"></i> Issued Keys</h4><h2 style="color:var(--neon)">{{ db.get('keys',{})|length }}</h2></div>
# [Auto-Cleaned CSS] # [Naked CSS Nuked]             <div class="card glass"><h4><i class="fa-solid fa-bolt"></i> Logs Triggered</h4><h2 style="color:var(--neon)">{{ db.get('logs',[])|length }}</h2></div>
        </div>
...
# [Auto-Cleaned CSS]         flash(f"Company '{name}' added successfully!")
    else:
...
# [Auto-Cleaned CSS]         flash(f"Company '{code}' updated successfully!")
    else:
...
# [Auto-Cleaned CSS]         flash(f"Company '{code}' deleted.")
    else:
...
# [Auto-Cleaned CSS]         # Query values ko smart dynamically replace kar do (e.g. ?pan=ABC -> ?pan={{pan}})
...
# [Auto-Cleaned CSS]             # Token mila toh encrypt karke save karlo, payload me {{token}} daal do
            if k == 'authorization' and v.lower().startswith('bearer '):
                res["auth_token"] = enc_token(v[7:].strip())
# [Auto-Cleaned CSS]                 res["headers"][k] = "Bearer {{token}}"
            else:
...
# [Auto-Cleaned CSS]         flash(f"Service '{code}' deleted.")
    else:
...
# [Auto-Cleaned CSS]         flash(f"New Key Generated Successfully: {k}")
    return redirect('/')
...
# [Auto-Cleaned CSS]         flash(f"Key {key_id} config updated!")
    else:
...
# [Auto-Cleaned CSS]         flash(f"Key {key_id} validity extended by {extra_days} days!")
    else:
        flash("Key not found!")
    return redirect('/')
...
# [Auto-Cleaned CSS]         flash(f"Key {key_id} deleted permanently!")
    return redirect('/')
...
# [Auto-Cleaned CSS]     db['keys'][key].setdefault('daily_usage', {})
...
def ping_services():
    db = GistDB.load()
# [Auto-Cleaned CSS]     services = db.get('services', {})
    updated = False
    now = now_ts()
...
# [Auto-Cleaned CSS]                 <p style="font-size:11px;opacity:0.6;margin-top:8px;">Pinged: {{ fmt_time(srv.health_last_check) }}</p>
            </div>
            {% endfor %}
        </div>
    </div>
...
# [Auto-Cleaned CSS]                     <tr><td>{{ c_id }}</td><td>{{ c.name }}</td><td>{{ c.tenant }} / {{ c.broker }}</td>
                        <td><span class="health-dot {% if c.active %}green{% else %}red{% endif %}"></span></td>
# [Auto-Cleaned CSS] # [Naked CSS Nuked]                         <td><a href="/admin/company/delete/{{ c_id }}" class="btn btn-danger" style="padding:4px 8px;font-size:12px;">Del</a></td></tr>
                    {% endfor %}
                </table>
            </div>
        </div>
...
# [Auto-Cleaned CSS]                 <tr><td>{{ s_id }}</td><td>{{ s.name }} ({{ s.company }})</td>
# [Auto-Cleaned CSS]                     <td><span style="font-size:11px;opacity:0.8">{{ s.method }}</span> {{ s.endpoint }}</td>
...
# [Auto-Cleaned CSS] # [Naked CSS Nuked]                     <td><a href="/admin/service/delete/{{ s_id }}" class="btn btn-danger" style="padding:4px 8px;font-size:12px;">Del</a></td></tr>
                {% endfor %}
            </table>
        </div>
    </div>
...
# [Auto-Cleaned CSS] # [Naked CSS Nuked]                             <a href="/admin/key/delete/{{ k_id }}" class="btn btn-danger" style="padding:4px 8px;font-size:12px;">Del</a></td></tr>
                    {% endfor %}
                </table>
            </div>
        </div>
...
# [Auto-Cleaned CSS]             <textarea id="curlInput" rows="7" placeholder="curl -X POST https://api... -H 'Authorization: Bearer xyz' -d '{...}'"></textarea>
...
# [Auto-Cleaned CSS] # [Naked CSS Nuked]                 <tr><td>{{ fmt_time(log.ts) }}</td><td style="font-family:monospace;color:var(--neon)">{{ log.key }}</td>
                    <td>{{ log.service }}</td><td><span class="health-dot {% if log.ok %}green{% else %}red{% endif %}"></span></td>
# [Auto-Cleaned CSS]                     <td style="font-size:12px;opacity:0.8">{{ log.msg }}</td></tr>
                {% endfor %}
            </table>
        </div>
    </div>
...
# [Auto-Cleaned CSS]     if(clock) clock.innerText = d.toLocaleTimeString('en-US', {hour12:true});
}, 1000);

async function processCurl() {
    let curl = document.getElementById('curlInput').value;
    let res = await fetch('/admin/service/import', {
# [Auto-Cleaned CSS]         method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'},
